[README.md](https://github.com/user-attachments/files/29795041/README.md)
...
- `find_best_k()`：遍历 K=1~10，计算每个 K 的 SSE（误差平方和）和轮廓系数
  - **肘部法则**：SSE 随 K 增大单调下降，下降变缓处为"肘部"
  - **轮廓系数**：取值范围 [-1, 1]，越接近 1 表示簇内紧凑、簇间分离
  - 选择轮廓系数最大的 K（跳过 K=1，因无法计算轮廓系数）
...
- **肘部法则**：随着 K 增大，SSE 逐渐下降但下降速率变慢，"肘部"处为最佳 K
- **轮廓系数**：衡量样本与其所在簇的紧密程度以及与其他簇的分离程度，取值越大越好
